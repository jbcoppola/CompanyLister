...
.filter('millionSuffix', function () {
    return function (input) {
        if (input.length > 8) {
            var cutInput = input.slice(0, input.length - 8);
            return cutInput + 'M';
        }
        return input;
    };
